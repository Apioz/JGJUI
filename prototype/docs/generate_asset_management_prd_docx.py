...
def build_body():
    ...
    parts.append(table(
        ...
        [
            ...
            ["大屏无入库出库", "完整流转仅中台"],
            ["左栏分类树", "空间/设备分类不联动图表"],
            ["底座平台", "office-mgmt/warehouse-mgmt 为主数据配置，详见《底座平台PRD》"],
    ...
    parts.append(h2("14. 附录 — officeAreaChart.allUnits（下钻「其他」单位）"))
    parts.append(p("summary 层 top5 见 9.2.1；以下为第 6 名及以后单位（detail 层展示）："))
    parts.append(table(
        ["单位", "占比%"],
        [
            ["区国资委", "3"], ["区级统筹闲置办公室", "3"],
